...
- `cargo build --features flutter --lib --release` - Build the patched desktop DLL used by the installed client
- `cargo build --features flutter --release --bin updesk_updater` - Build the Windows updater helper binary
...
- `.\deploy_local_client.ps1` - Copy the built `libuptimedesk.dll` into the installed UptimeDesk app and restart it
- `python3 deploy_updesk_server.py` - Publish relay bridge/proxy files to `updesk.uptimeservice.it`
...
### Key Components
- **Remote Desktop Protocol**: Custom protocol implemented in `src/rendezvous_mediator.rs` for communicating with uptimedesk-server

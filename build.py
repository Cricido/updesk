...
hbb_name = 'uptimedesk' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: uptimedesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: uptimedesk <info@rustdesk.com>
...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/UptimeDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"UptimeDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon UptimeDesk.app 200 190 --hide-extension UptimeDesk.app rustdesk.dmg ./build/macos/Build/Products/Release/UptimeDesk.app")
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/libuptimedesk.dll"):
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/rustdesk.exe target/release/UptimeDesk.exe')
        pa = os.environ.get('P')
        if pa:
            ...
            print('Not signed')
        system2(
            f'cp -rf target/release/UptimeDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
        ...
        system2('mv uptimedesk-%s-0-x86_64.pkg.tar.zst uptimedesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        ...
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/UptimeDesk.app/Contents/MacOS/rustdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/UptimeDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/UptimeDesk.app/Contents/MacOS/rustdesk
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/UptimeDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/UptimeDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/UptimeDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/UptimeDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "UptimeDesk %s.dmg" "target/release/bundle/osx/UptimeDesk.app"' % version)
                os.rename('UptimeDesk %s.dmg' %
                          version, 'uptimedesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple uptimedesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/UptimeDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                ...
                os.rename('rustdesk.deb', 'uptimedesk-%s.deb' % version)

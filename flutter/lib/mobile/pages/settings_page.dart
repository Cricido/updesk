...
class _SettingsState extends State<SettingsPage> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
      ...
      enhancementsTiles.insert(
          ...
          SettingsTile.switchTile(
              ...
              title: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(translate('Keep UpDesk background service')),
    ...
    String readUpdatePolicy() {
      ...
      if (!checkEnabled) return 'disabled';
      if (channel == 'beta') return 'beta';
      if (channel == 'recommended' || autoEnabled) return 'recommended';
      return 'stable';
    }

    Future<void> saveUpdatePolicy(String policy) async {
      switch (policy) {
        case 'disabled':
          ...
          await bind.mainSetLocalOption(
              key: kOptionUpdateChannel, value: 'stable');
          break;
        case 'stable':
          await bind.mainSetLocalOption(
              key: kOptionEnableCheckUpdate, value: 'Y');
          await bind.mainSetOption(key: kOptionAllowAutoUpdate, value: 'N');
          await bind.mainSetLocalOption(
              key: kOptionUpdateChannel, value: 'stable');
          break;
        case 'recommended':
          ...
          await bind.mainSetLocalOption(
              key: kOptionUpdateChannel, value: 'recommended');
          break;
        case 'beta':
          await bind.mainSetLocalOption(
              key: kOptionEnableCheckUpdate, value: 'Y');
          await bind.mainSetOption(key: kOptionAllowAutoUpdate, value: 'N');
          await bind.mainSetLocalOption(
              key: kOptionUpdateChannel, value: 'beta');
          break;
      }
      setState(() {
    ...
    enhancementsTiles.add(_getPopupDialogRadioEntry(
      title: 'Updates',
      list: [
        _RadioEntry('Disattivato', 'disabled'),
        _RadioEntry('Canale stabile', 'stable'),
        _RadioEntry('Aggiornamenti consigliati', 'recommended'),
        _RadioEntry('Canale beta', 'beta'),
      ],
      getter: readUpdatePolicy,
      asyncSetter: saveUpdatePolicy,
    ));

    enhancementsTiles.add(SettingsTile(
      title: Text(translate('Check for software update on startup')),
      value: Text(
        _checkUpdateOnStartup ? translate('Enabled') : translate('Disabled'),
      ),
      onPressed: (context) async {
        final toValue = !_checkUpdateOnStartup;
    ...
    enhancementsTiles.add(SettingsTile(
      title: Text(translate('Check for updates now')),
        ...
        SettingsSection(
          ...
          tiles: [
            ...
            SettingsTile(
              ...
              onPressed: (context) =>
                  launchUrlString('https://updesk.it/privacy'),
...
void showAbout(OverlayDialogManager dialogManager) {
  dialogManager.show((setState, close, context) {
    return CustomAlertDialog(
      title: Text(translate('About UpDesk')),

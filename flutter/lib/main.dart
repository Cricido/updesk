...
import 'common.dart';
import 'consts.dart';
import 'new_ui/quick_support_page.dart';
  ...
  if (args.isNotEmpty && args.first == 'multi_window') {
    ...
    switch (kWindowType) {
      ...
      case WindowType.Terminal:
        ...
        break;
      case WindowType.QuickSupport:
        desktopType = DesktopType.main;
        runQuickSupportWindow();
        break;
      default:
        break;
    }
...
void runMainApp(bool startService) async {
  ...
  // Default approve mode to 'click' (popup ACCETTA/RIFIUTA) on first run
  final currentApproveMode = bind.mainGetOptionSync(key: kOptionApproveMode);
  if (currentApproveMode.isEmpty) {
  ...
  windowManager.waitUntilReadyToShow(windowOptions, () async {
    ...
    if (handledByUniLinks ||
        ...
        kBootArgs.contains('--minimized')) {
      windowManager.hide();
    } else {
      windowManager.show();
      windowManager.focus();
      // Move registration of active main window here to prevent from async visible check.
      winManager.registerActiveWindow(kWindowMainId);
    }
...
void runQuickSupportWindow() async {
  final botToastBuilder = BotToastInit();
  _runApp(
    'UptimeDesk – Assistenza',
    const QuickSupportPage(),
    MyTheme.currentThemeMode(),
  );
  WindowController.fromWindowId(kWindowId!).show();
}

void runQuickSupportApp() async {
  await initEnv(kAppTypeMain);
  await bind.mainCheckConnectStatus();
  gFFI.serverModel.startService();
  final botToastBuilder = BotToastInit();
  runApp(GetMaterialApp(
    ...
    title: 'UptimeDesk – Assistenza',
    theme: MyTheme.lightTheme,
    darkTheme: MyTheme.darkTheme,
    themeMode: MyTheme.currentThemeMode(),
    home: const QuickSupportPage(),
  ...
  windowManager.waitUntilReadyToShow(windowOptions, () async {
    windowManager.setPosition(const Offset(80, 80));
    windowManager.show();
    windowManager.focus();

...
Widget buildConnectionCard(Client client) {
  return Consumer<ServerModel>(
    builder: (context, value, child) {
      final isPending = !client.authorized && !client.disconnected;
      return Container(
        color: isPending ? UdTheme.bg : null,
        child: Column(
          ...
          children: [
            ...
            // Permission toggles hidden — too technical for end users
            Expanded(
              child: Align(
          ...
          vertical: isPending ? 0.0 : 4.0,
          horizontal: isPending ? 0.0 : 8.0,
        ),
      );
    },
  );
...
class _CmHeaderState extends State<_CmHeader>
  ...
  Widget build(BuildContext context) {
    super.build(context);
    if (!client.authorized && !client.disconnected) {
      return _buildPendingHeader(context);
    }
    return Container(
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.shield_outlined, color: UdTheme.green, size: 42),
          ),
          const SizedBox(height: 18),
          Text(
            client.name,
            style: const TextStyle(
              color: UdTheme.textPrimary,
              fontSize: 20,
              fontWeight: FontWeight.bold,
            ),
          ...
          Text(
            translate("Request access to your device"),
            style: const TextStyle(color: UdTheme.textSecondary, fontSize: 14),
            textAlign: TextAlign.center,
          ),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: UdTheme.green.withOpacity(0.10),
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: UdTheme.green.withOpacity(0.30)),
            ),
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                const Icon(Icons.lock_outline, color: UdTheme.green, size: 13),
                const SizedBox(width: 6),
                Flexible(
                  child: Text(
                    translate("Protected connection (end-to-end encrypted)"),
                    style: const TextStyle(color: UdTheme.green, fontSize: 11),
                    textAlign: TextAlign.center,
                  ),
                ),
              ],
...
class _CmControlPanelState extends State<_CmControlPanel> {
  ...
  void initState() {
    ...
    if (!client.authorized) {
      _countdownTimer = Timer.periodic(const Duration(seconds: 1), (t) {
        ...
        if (client.authorized) { t.cancel(); return; }
        setState(() {
          _countdown--;
          if (_countdown <= 0) {
            t.cancel();
          ...
          child: Column(
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const Icon(Icons.timer_outlined, size: 13, color: UdTheme.textSecondary),
                  const SizedBox(width: 4),
                  Text(
                    "Chiusura tra ${_countdown}s",
                    style: const TextStyle(color: UdTheme.textSecondary, fontSize: 12),
                  ),
                ],
              ),
              ...
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  ...
                  minHeight: 4,
                  backgroundColor: UdTheme.surfaceBorder,
                  valueColor: AlwaysStoppedAnimation<Color>(
                    _countdown > 10 ? UdTheme.navy : Colors.orange,
                  ),
                ),
              ),
            ],
  ...
  Widget _buildPrimaryButton(
    ...
    final btn = Container(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(10),
        onTap: () => checkClickTime(client.id, onTap),
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            if (icon != null) ...[

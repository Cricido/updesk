    ...
    for (final tab in DesktopSettingPage.tabKeys) {
      switch (tab) {
        ...
        case SettingsTabKey.updates:
          settingTabs.add(_TabInfo(tab, 'Updates',
    ...
    return Scaffold(
      ...
      body: _buildBlock(
        children: <Widget>[
          ...
          Container(width: 1, color: ud.surfaceBorder),
          Expanded(
            child: Container(
              color: ud.bg,
              child: PageView(
  ...
  Widget _header(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border(bottom: BorderSide(color: ud.surfaceBorder)),
      ),
      child: Row(
        ...
        children: [
          if (isWeb)
            IconButton(
              ...
              icon: Icon(Icons.arrow_back, color: ud.textSecondary, size: 18),
              padding: EdgeInsets.zero,
              constraints: const BoxConstraints(),
            ).marginOnly(right: 8),
          const Icon(Icons.settings_outlined, color: UdTheme.navy, size: 18),
          const SizedBox(width: 9),
          Text(
            translate('Settings'),
            style: TextStyle(
              color: ud.textPrimary,
              fontSize: 16,
      ...
      return Padding(
        ...
        child: InkWell(
          ...
          child: AnimatedContainer(
            ...
            height: _kTabHeight - 6,
            padding: const EdgeInsets.symmetric(horizontal: 12),
            decoration: BoxDecoration(
              color: selected ? UdTheme.navy.withOpacity(0.12) : Colors.transparent,
              borderRadius: BorderRadius.circular(8),
            ),
            child: Row(children: [
              Icon(
                selected ? tab.selected : tab.unselected,
                color: selected ? UdTheme.navy : ud.textSecondary,
                size: 18,
              ),
...
class _UpdatesState extends State<_Updates> {
  ...
  void initState() {
    super.initState();
    _loadVersion();
    _refreshTimer = periodic_immediate(const Duration(seconds: 1), _refreshState);
  }
  ...
  Future<void> _refreshState() async {
    ...
    setState(() {
      _remoteVersion = bind.mainGetCommonSync(key: 'software-update-version').trim();
      _changelog = bind.mainGetCommonSync(key: 'software-update-changelog').trim();
      _minSupported =
          bind.mainGetCommonSync(key: 'software-update-min-supported').trim();
      _status = bind.mainGetCommonSync(key: 'software-update-status').trim();
      _error = bind.mainGetCommonSync(key: 'software-update-error').trim();
      _mandatory =
          bind.mainGetCommonSync(key: 'software-update-mandatory').trim() == 'true';
    });
  }
  ...
  _UpdatePolicy _readPolicy() {
    ...
    final autoEnabled = mainGetBoolOptionSync(kOptionAllowAutoUpdate);
    if (!checkEnabled) {
      return _UpdatePolicy.disabled;
    }
    if (channel == 'beta') {
      return _UpdatePolicy.beta;
    }
    if (channel == 'recommended') {
      return _UpdatePolicy.recommended;
    }
    if (autoEnabled) {
      return _UpdatePolicy.recommended;
    }
    return _UpdatePolicy.stable;
  }

  Future<void> _savePolicy(_UpdatePolicy policy) async {
    switch (policy) {
      case _UpdatePolicy.disabled:
        ...
        await bind.mainSetLocalOption(key: kOptionUpdateChannel, value: 'stable');
        break;
      case _UpdatePolicy.stable:
        await bind.mainSetLocalOption(key: kOptionEnableCheckUpdate, value: 'Y');
        await bind.mainSetOption(key: kOptionAllowAutoUpdate, value: 'N');
        await bind.mainSetLocalOption(key: kOptionUpdateChannel, value: 'stable');
        break;
      case _UpdatePolicy.recommended:
        await bind.mainSetLocalOption(key: kOptionEnableCheckUpdate, value: 'Y');
        await bind.mainSetOption(key: kOptionAllowAutoUpdate, value: 'Y');
        await bind.mainSetLocalOption(key: kOptionUpdateChannel, value: 'recommended');
        break;
      case _UpdatePolicy.beta:
        await bind.mainSetLocalOption(key: kOptionEnableCheckUpdate, value: 'Y');
        await bind.mainSetOption(key: kOptionAllowAutoUpdate, value: 'N');
        await bind.mainSetLocalOption(key: kOptionUpdateChannel, value: 'beta');
        break;
    }
    await _refreshState();
    setState(() {});
  ...
  Future<void> _runManualCheck() async {
    if (_readPolicy() == _UpdatePolicy.disabled) {
  ...
  Future<void> _installNow() async {
    final updateUrl = bind.mainGetCommonSync(key: 'software-update-url').trim();
  ...
  String _statusLabel() {
    ...
    switch (_status) {
      ...
      default:
        return 'Nessun controllo recente.';
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final currentChannel =
        bind.mainGetLocalOption(key: kOptionUpdateChannel).trim().isEmpty
            ? 'stable'
            : bind.mainGetLocalOption(key: kOptionUpdateChannel).trim();
    final currentManifest = currentChannel == 'beta'
        ? 'https://updesk.uptimeservice.it/api/v1/update/beta.json'
        : currentChannel == 'recommended'
    ...
    return ListView(
      controller: ScrollController(),
      children: [
        _Card(title: 'Aggiornamenti', children: [
          _Radio<_UpdatePolicy>(
            context,
            value: _UpdatePolicy.disabled,
            groupValue: policy,
            label: 'Disattivato',
            onChanged: _savePolicy,
          ),
          _Radio<_UpdatePolicy>(
            context,
            value: _UpdatePolicy.stable,
            groupValue: policy,
            label: 'Canale stabile',
            onChanged: _savePolicy,
          ),
          _Radio<_UpdatePolicy>(
            context,
            value: _UpdatePolicy.recommended,
            groupValue: policy,
            label: 'Aggiornamenti consigliati',
            onChanged: _savePolicy,
          ),
          _Radio<_UpdatePolicy>(
            context,
            value: _UpdatePolicy.beta,
            groupValue: policy,
            label: 'Canale beta',
            onChanged: _savePolicy,
          ),
        ]),
        _Card(title: 'Stato update', children: [
          SelectionArea(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Versione attuale: ${_currentVersion.isEmpty ? "-" : _currentVersion}')
                    .marginOnly(left: _kContentHMargin, top: 4, bottom: 4),
                Text('Versione remota: ${_remoteVersion.isEmpty ? "-" : _remoteVersion}')
                    .marginOnly(left: _kContentHMargin, top: 4, bottom: 4),
                Text('Manifest attivo: $currentManifest')
                    .marginOnly(left: _kContentHMargin, top: 4, bottom: 4),
                Text(
                  'Stato: ${_statusLabel()}',
                  style: TextStyle(
                    color: _error.isNotEmpty ? Colors.redAccent : null,
                  ),
                ).marginOnly(left: _kContentHMargin, top: 4, bottom: 4),
                if (_mandatory)
                  Text('Aggiornamento obbligatorio')
                      .marginOnly(left: _kContentHMargin, top: 4, bottom: 4),
                if (_minSupported.isNotEmpty)
                  Text('Versione minima supportata: $_minSupported')
                      .marginOnly(left: _kContentHMargin, top: 4, bottom: 4),
                if (_changelog.isNotEmpty)
                  Text('Changelog: $_changelog')
                      .marginOnly(left: _kContentHMargin, top: 4, bottom: 4),
              ],
            ),
          ),
        ]),
        _Card(title: 'Azioni', children: [
          Row(
            children: [
              _Button(
              ...
              _Button(
                'Riavvia e installa',
                _installNow,
                enabled: !_isBusy && _remoteVersion.isNotEmpty,
              ),
            ],
          ).marginOnly(left: _kContentHMargin),
        ]),
...
class _SafetyState extends State<_Safety> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> _loadUnattended() async {
    final pwd = await bind.mainGetPermanentPassword();
    final mode = bind.mainGetOptionSync(key: 'approve-mode');
    if (mounted) {
      setState(() => _unattendedEnabled = mode == 'password' && pwd.isNotEmpty);
    }
  }

  Future<void> _toggleUnattended(bool value) async {
    if (value) {
      setPasswordDialog(notEmptyCallback: () async {
        await bind.mainSetOption(key: 'approve-mode', value: 'password');
        await bind.mainSetOption(key: 'verification-method', value: kUsePermanentPassword);
        await gFFI.serverModel.updatePasswordModel();
        if (mounted) setState(() => _unattendedEnabled = true);
      });
    } else {
      ...
      await gFFI.serverModel.updatePasswordModel();
      if (mounted) setState(() => _unattendedEnabled = false);
    }
  }

  Widget _unattendedCard(BuildContext context) {
    final bool enabled = !locked;
    return _Card(title: 'Accesso non vigilato', children: [
      Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Il tecnico si connette con password senza bisogno del tuo click.',
                        style: TextStyle(
                          fontSize: 12,
                ...
                Switch(
                  ...
                  onChanged: enabled ? _toggleUnattended : null,
                  activeColor: const Color(0xFF3CC870),
                ),
              ],
            ),
            if (_unattendedEnabled) ...[
              ...
              ElevatedButton.icon(
                onPressed: enabled ? setPasswordDialog : null,
                icon: const Icon(Icons.lock_reset_rounded, size: 16),
                label: const Text('Cambia password'),
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFF3CC870),
...
class _AboutState extends State<_About> {
  ...
  Widget build(BuildContext context) {
    return futureBuilder(future: () async {
      ...
      return SingleChildScrollView(
        controller: scrollController,
        child: _Card(title: translate('About UpDesk'), children: [
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const SizedBox(
              ...
              Row(
                children: [
                  InkWell(
                      onTap: () {
                        launchUrlString('https://www.uptimeservice.it/privacy-policy/');
                      },
                      child: Text(
                  ...
                  InkWell(
                      onTap: () {
                        launchUrlString('https://www.uptimeservice.it/');
                      },
                      child: Text(
                        translate('Website'),
                        style: linkStyle,
              ...
              Container(
                decoration: const BoxDecoration(color: UdTheme.navy),
                padding:
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            'Copyright © ${DateTime.now().toString().substring(0, 4)} MED srl.\n$license',
                            style: const TextStyle(color: Colors.white),
                          ),
                          const SizedBox(height: 12),
                          Text(
                            'La distanza non è un limite, se il metodo è solido!',
                            style: TextStyle(
  ...
  return Builder(builder: (context) {
    ...
    return Row(
      children: [
        Flexible(
          child: Container(
              margin: EdgeInsets.only(left: _kCardLeftMargin, top: 15, right: _kCardLeftMargin),
              decoration: BoxDecoration(
                color: ud.surface,
                borderRadius: BorderRadius.circular(UdTheme.radMd),
                border: Border.all(color: ud.surfaceBorder),
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Container(
                    padding: EdgeInsets.symmetric(
                        horizontal: _kContentHMargin, vertical: 12),
                    decoration: BoxDecoration(
                      border: Border(
                          bottom: BorderSide(color: ud.surfaceBorder)),
                      borderRadius: const BorderRadius.vertical(
                          top: Radius.circular(UdTheme.radMd)),
                    ),
                    child: Row(
                      children: [
                        Expanded(
...
class _WaylandCardState extends State<WaylandCard> {
  ...
  //   "Settings" -> "Apps" -> "UpDesk" -> "Permissions" -> "Inhibit Shortcuts".
  // For resetting(clearing) the permission from the portal permission store, you can
  // use (replace <desktop-id> with the UpDesk desktop file ID):
  ...
  // On a native install this is typically "updesk.desktop"; on Flatpak it is usually
  // the exported desktop ID derived from the Flatpak app-id (e.g. "org.updesk.UpDesk.desktop").
  //

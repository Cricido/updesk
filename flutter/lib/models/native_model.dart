...
class PlatformFFI {
  ...
  UptimedeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libuptimedesk.so')
        : isLinux
            ? DynamicLibrary.open('libuptimedesk.so')
            : isWindows
                ? DynamicLibrary.open('libuptimedesk.dll')
                :
  ...
  void _startListenEvent(UptimedeskImpl uptimedeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = uptimedeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {

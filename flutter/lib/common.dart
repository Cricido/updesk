...
class MyTheme {
  ...
  static const Color accent = Color(0xFF3CC870);    // UpDesk green (visible light+dark)
  static const Color accent50 = Color(0x773CC870);
  static const Color accent80 = Color(0xAA3CC870);
  static const Color canvasColor = Color(0xFF212121);
  static const Color border = Color(0xFFCCCCCC);
  static const Color idColor = Color(0xFF3CC870);   // UpDesk green
  static const Color darkGray = Color.fromARGB(255, 148, 148, 148);
  static const Color cmIdColor = Color(0xFF1A8040);
  static const Color dark = Colors.black87;
  static const Color button = Color(0xFF3CC870);    // UpDesk green
  ...
  static ThemeData lightTheme = ThemeData(
    ...
    menuBarTheme: MenuBarThemeData(
        style:
            MenuStyle(backgroundColor: MaterialStatePropertyAll(Color(0xF2F3F6FA)))),
    colorScheme: ColorScheme.light(
        primary: accent, secondary: const Color(0xFF1C3D6B), background: grayBg),
  ...
  static ThemeData darkTheme = ThemeData(
    ...
    menuBarTheme: MenuBarThemeData(
        style: MenuStyle(
            backgroundColor: MaterialStatePropertyAll(Color(0xF2161B25)))),
...
Future<void> windowOnTop(int? id) async {
  ...
  if (id == null) {
    ...
    await winManager.registerActiveWindow(kWindowMainId);
  } else {
    WindowController.fromWindowId(id)
      ..focus()
      ..show();
    winManager.call(WindowType.Main, kWindowEventShow, {"id": id});
  }
}
...
Future<bool> restoreWindowPosition(WindowType type,
  ...
  if (bind
      .mainGetEnv(key: "DISABLE_UPTIMEDESK_RESTORE_WINDOW_POSITION")
      .isNotEmpty) {
    return false;
  }
  if (type != WindowType.Main && windowId == null) {
  ...
  if (type != null && id != null) {
    switch (type) {
      case UriLinkType.remoteDesktop:
        Future.delayed(Duration.zero, () {
          winManager.newRemoteDesktop(id!,
              password: password,
      ...
      case UriLinkType.fileTransfer:
        Future.delayed(Duration.zero, () {
          winManager.newFileTransfer(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.viewCamera:
        Future.delayed(Duration.zero, () {
          winManager.newViewCamera(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.portForward:
        Future.delayed(Duration.zero, () {
          winManager.newPortForward(id!, false,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.rdp:
        Future.delayed(Duration.zero, () {
          winManager.newPortForward(id!, true,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.terminal:
        Future.delayed(Duration.zero, () {
          winManager.newTerminal(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
    }
...
List<String>? urlLinkToCmdArgs(Uri uri) {
  ...
  if (uri.authority.isEmpty &&
    ...
    // uptimedesk://<connect-id>
    // uptimedesk://<connect-id>/r
    // uptimedesk://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
...
connectMainDesktop(String id,
  ...
  if (isFileTransfer) {
    await winManager.newFileTransfer(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isViewCamera) {
    await winManager.newViewCamera(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isTcpTunneling || isRDP) {
    await winManager.newPortForward(id, isRDP,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isTerminal) {
    await winManager.newTerminal(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else {
    await winManager.newRemoteDesktop(id,
        password: password,
        isSharedPassword: isSharedPassword,
        forceRelay: forceRelay);
  }
...
connect(BuildContext context, String id,
  ...
  if (isDesktop) {
    if (desktopType == DesktopType.main) {
      ...
      await winManager.call(WindowType.Main, kWindowConnect, {
        'id': id,
        'isFileTransfer': isFileTransfer,
        'isViewCamera': isViewCamera,
        'isTerminal': isTerminal,
...
Future<void> onActiveWindowChanged() async {
  print(
      "[MultiWindowHandler] active window changed: ${winManager.getActiveWindows()}");
  if (winManager.getActiveWindows().isEmpty) {
    // close all sub windows
    try {
      if (isLinux) {
        await Future.wait([
          saveWindowPosition(WindowType.Main),
          winManager.closeAllSubWindows()
        ]);
      } else {
      ...
      debugPrint("Start closing UptimeDesk...");
      ...
      if (isMacOS) {
        ...
        // 2024-11-11 11:41:11.546 UptimeDesk[90272:2567686] Failed to create a FlutterPlatformMessageResponseHandle (2)
        // embedder.cc (2672): 'FlutterEngineSendPlatformMessage' returned 'kInvalidArguments'. Invalid engine handle.
        // 2024-11-11 11:41:11.565 UptimeDesk[90272:2567686] Failed to send message to Flutter engine on channel 'flutter/lifecycle' (2).
        // ```
        periodic_immediate(
...
class ServerConfig {
  ...
  /// decode from shared string (from user shared or uptimedesk-server generated)
  /// also see [encode]
...
const kMacOSPermChannel = MethodChannel("org.updesk.updesk/host");
...
Future<List<Rect>> getScreenListWayland() async {
  ...
  if (isMainDesktopWindow) {
    ...
    final screenList = await winManager.call(
...
Widget loadPowered(BuildContext context) {
  ...
  return MouseRegion(
    ...
    child: GestureDetector(
      onTap: () {
        launchUrl(Uri.parse('https://updesk.it'));
      },
      child: Opacity(
          opacity: 0.5,
          child: Text(
...
String _lastGlobalUpdatePromptUrl = '';

void _maybeShowGlobalUpdatePrompt() {
  final updateUrl = stateGlobal.updateUrl.value;
  final isInstalledDesktop = (isWindows || isMacOS) && bind.mainIsInstalled();
  if (!(isDesktop &&
      desktopType == DesktopType.main &&
      isInstalledDesktop &&
      updateUrl.isNotEmpty &&
      updateUrl != _lastGlobalUpdatePromptUrl)) {
    return;
  }
  _lastGlobalUpdatePromptUrl = updateUrl;
  final version = stateGlobal.updateVersion.value.isNotEmpty
      ? stateGlobal.updateVersion.value
      : bind.mainGetNewVersion();
  final mandatory = stateGlobal.updateMandatory.value;
  final minSupported = stateGlobal.updateMinSupported.value;
  final changelog = stateGlobal.updateChangelog.value.trim();
  Future.delayed(const Duration(milliseconds: 500), () {
    gFFI.dialogManager.show((setState, close, context) {
      return CustomAlertDialog(
        ...
        content: ConstrainedBox(
          ...
          child: SelectionArea(
            child: Column(
              ...
              children: [
                Text(
                  '${bind.mainGetAppNameSync()} $version è disponibile.',
                ...
                if (mandatory)
                  const Padding(
                    ...
                    child: Text(
                      'Questo aggiornamento è richiesto.',
                ...
                if (minSupported.isNotEmpty)
                  Padding(
                    ...
                    child: Text('Versione minima supportata: $minSupported'),
                  ),
        ...
        actions: [
          if (!mandatory)
            dialogButton('Più tardi', onPressed: close, isOutline: true),
          dialogButton('Riavvia e installa', onPressed: () {
            close();
            bind.mainSetCommon(key: 'update-me', value: updateUrl);
            Future.delayed(const Duration(milliseconds: 300), _syncUpdateStateFromCoreCache);
          }),
        ],
      );
    }, tag: 'global-update-prompt-$version');
  });
}

Future<void> _syncUpdateStateFromCoreCache() async {
  final updateUrl = bind.mainGetCommonSync(key: 'software-update-url').trim();
  stateGlobal.updateUrl.value = updateUrl;
  stateGlobal.updateVersion.value =
      bind.mainGetCommonSync(key: 'software-update-version').trim();
  stateGlobal.updateChangelog.value =
      bind.mainGetCommonSync(key: 'software-update-changelog').trim();
  stateGlobal.updateMandatory.value =
      bind.mainGetCommonSync(key: 'software-update-mandatory').trim() == 'true';
  stateGlobal.updateMinSupported.value =
  ...
  stateGlobal.updateError.value =
      bind.mainGetCommonSync(key: 'software-update-error').trim();
  if (updateUrl.isEmpty) {
    return;
  }
  _maybeShowGlobalUpdatePrompt();
}
...
void checkUpdate() {
  if (!isWeb) {
    platformFFI.registerEventHandler(
      ...
      stateGlobal.updateVersion.value =
          evt['version'] is String ? evt['version'] : '';
      stateGlobal.updateChangelog.value =
          evt['changelog'] is String ? evt['changelog'] : '';
      stateGlobal.updateMandatory.value =
          (evt['mandatory']?.toString().toLowerCase() == 'true');
      stateGlobal.updateMinSupported.value =
          evt['min_supported'] is String ? evt['min_supported'] : '';
      _maybeShowGlobalUpdatePrompt();
    });
    Timer(const Duration(seconds: 1), () async {
      bind.mainGetSoftwareUpdateUrl();
      Future.delayed(const Duration(seconds: 2), _syncUpdateStateFromCoreCache);
      Future.delayed(const Duration(seconds: 5), _syncUpdateStateFromCoreCache);
    });
  }
}

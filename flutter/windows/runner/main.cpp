...
{
  HINSTANCE hInstance = LoadLibraryA("libuptimedesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libuptimedesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_RUSTDESK_CORE_MAIN rustdesk_core_main =
      (FUNC_RUSTDESK_CORE_MAIN)GetProcAddress(hInstance, "uptimedesk_core_main_args");
  if (!rustdesk_core_main)
  {
    std::cout << "Failed to get uptimedesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (!c_args)
  {
    ...
    // std::cout << "UpDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"UpDesk";
  FUNC_RUSTDESK_GET_APP_NAME get_app_name = (FUNC_RUSTDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_uptimedesk_app_name");
  if (get_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_app_name(app_name_buffer, 512) == 0) {

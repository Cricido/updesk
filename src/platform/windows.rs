...
pub fn get_custom_client_staging_dir() -> PathBuf {
    get_public_base_dir()
        .join("UptimeDesk")
        .join("UptimeDeskCustomClientStaging")
}
...
/// - Deleting directories under a public location (e.g., C:\\ProgramData\\UptimeDesk) is
...
pub fn bootstrap() -> bool {
    ...
    #[cfg(not(debug_assertions))]
    {
        // This function will cause `'sciter.dll' was not found neither in PATH nor near the current executable.` when debugging UptimeDesk.
...
mod cert {
    ...
    extern "C" {
        fn DeleteUptimeDeskTestCertsW();
    }
    pub fn uninstall_cert() -> ResultType<()> {
        unsafe {
            DeleteUptimeDeskTestCertsW();
        }
        Ok(())
    }
}
...
reg add {subkey} /f /v EstimatedSize /t REG_DWORD /d {size}
    ...
    // 1. There may be some other processes like `uptimedesk --connect` are running.
...
pub fn update_to(file: &str) -> ResultType<()> {
    if file.ends_with(".exe") {
        ...
        log::info!("launching update exe via UAC: {}", file);
        if !run_uac(file, "--update")? {
            log::warn!(
                "failed to run update exe with --update via UAC, retrying without arguments: file={} last_error={:?}",
                file,
                std::io::Error::last_os_error()
            );
            if !run_uac(file, "")? {
    ...
    for entry in entries {
        if let Ok(entry) = entry {
            ...
            if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                // Match files like uptimedesk-*.msi or uptimedesk-*.exe
                if file_name.starts_with("uptimedesk-")
...
pub fn try_kill_uptimedesk_main_window_process() -> ResultType<()> {
    ...
    bail!("failed to find uptimedesk main window process");
}
...
fn get_pids_with_args_from_wmic_output<S2: AsRef<str>>(
    ...
    // ProcessId=34668
    //
    // CommandLine="C:\Program Files\UptimeDesk\UptimeDesk.exe" --tray
    // ProcessId=13728
    //
    // CommandLine="C:\Program Files\UptimeDesk\UptimeDesk.exe"
    // ProcessId=10136
    let mut pids = Vec::new();

...
lazy_static::lazy_static! {
    ...
    static ref SUDO_E_PRESERVES_ENV: bool = {
        if !is_root() {
            ...
            let key = format!("__UPTIMEDESK_SUDO_E_TEST_{}", std::process::id());
            let val = "1";
...
// stop_uptimedesk_servers() is just a temp solution here.
fn force_stop_server() {
    stop_uptimedesk_servers();
...
mod desktop {
    ...
    pub struct Desktop {
        ...
        pub home: String,
        pub dbus: String,
        pub is_uptimedesk_subprocess: bool,
    ...
    impl Desktop {
        ...
        pub fn is_headless(&self) -> bool {
            self.sid.is_empty() || self.is_uptimedesk_subprocess
        }
        ...
        fn set_is_subprocess(&mut self) {
            self.is_uptimedesk_subprocess = false;
            let cmd = format!(
            ...
            if let Ok(res) = run_cmds(&cmd) {
                if res.trim() != "0" {
                    self.is_uptimedesk_subprocess = true;
                }
            }
        }
...
pub fn uninstall_service(show_new_window: bool, _: bool) -> bool {
    ...
    // systemctl kill uptimedesk --tray, execute cp first
...
fn get_shortcuts_inhibitor_app_id() -> String {
    if is_flatpak() {
        ...
        // (e.g., "com.rustdesk.UptimeDesk"). This is the most reliable source.

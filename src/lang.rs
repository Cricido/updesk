...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_uptimedesk() {
            if s.contains("UptimeDesk")
                && !name.starts_with("upgrade_uptimedesk_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("UptimeDesk") {
                    s = s.replace("UptimeDesk", &app_name);
                } else {
                    // https://github.com/rustdesk/rustdesk-server-pro/issues/845
                    // If app_name contains "UptimeDesk" (e.g., "UptimeDesk-Admin"), we need to avoid
                    // replacing "UptimeDesk" within the already-substituted app_name, which would
                    // cause duplication like "UptimeDesk-Admin" -> "UptimeDesk-Admin-Admin".
                    //

                ...
                match msg_in.union {
                    ...
                    Some(rendezvous_message::Union::PunchHoleResponse(ph)) => {
                        if !ph.socket_addr.is_empty() {
                            ...
                            let fallback_relay_server = if ph.relay_server.is_empty() {
                                relay_server.clone()
                            } else {
                                ph.relay_server.clone()
                            };
                            if peer_addr.ip().is_loopback() {
                                if fallback_relay_server.is_empty() {
                                    bail!(
                                        "legacy request_relay fallback returned loopback address without relay server"
                                    );
                                }
                                log::warn!(
                                    "legacy request_relay fallback returned loopback addr {:?}, force relay via {}",
                                    peer_addr,
                            ...
                            log::info!(
                                "legacy request_relay fallback returned PunchHoleResponse, try direct tcp to {:?}",
                                peer_addr
                            );
                            return connect_tcp_local(peer_addr, None, CONNECT_TIMEOUT).await;
                        }
                        if !ph.other_failure.is_empty() {
                            bail!(ph.other_failure);
                        }
    ...
    fn check_clipboard(&mut self) {
        if CLIPBOARD_STATE.lock().unwrap().running {
            ...
            if let Some(urls) = check_clipboard_files(&mut self.ctx, ClipboardSide::Client, false) {
                if !urls.is_empty() {
                    #[cfg(target_os = "macos")]
                    if crate::clipboard::is_file_url_set_by_uptimedesk(&urls) {
                        return;
                    }

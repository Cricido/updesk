...
pub const PORTABLE_APPNAME_RUNTIME_ENV_KEY: &str = "UPTIMEDESK_APPNAME";
...
lazy_static::lazy_static! {
    ...
    pub static ref SOFTWARE_UPDATE_MANIFEST: Arc<Mutex<Option<SoftwareUpdateManifest>>> = Default::default();
    pub static ref DEVICE_ID: Arc<Mutex<String>> = Default::default();
    pub static ref DEVICE_NAME: Arc<Mutex<String>> = Default::default();
    static ref PUBLIC_IPV6_ADDR: Arc<Mutex<(Option<SocketAddr>, Option<Instant>)>> = Default::default();
}

pub const SOFTWARE_UPDATE_MANIFEST_URL: &str =
    "https://updesk.uptimeservice.it/api/v1/update/stable.json";
pub const SOFTWARE_UPDATE_RECOMMENDED_MANIFEST_URL: &str =
...
pub struct SoftwareUpdateManifest {
    #[serde(default)]
    pub channel: String,
    #[serde(default)]
    pub version: String,
...
fn get_software_update_channel() -> String {
    let channel = LocalConfig::get_option(OPTION_UPDATE_CHANNEL);
    if channel.eq_ignore_ascii_case("beta") {
        "beta".to_owned()
    } else if channel.eq_ignore_ascii_case("recommended") {
...
fn validate_software_update_manifest(
    ...
    if parsed.scheme() != "https" {
        bail!("Invalid update manifest: update url must use https");
    }
    if parsed.host_str() != Some("updesk.uptimeservice.it") {
        bail!("Invalid update manifest: unexpected update host");
    }
    if !parsed.path().starts_with("/releases/") {
        bail!("Invalid update manifest: unexpected update path");
    }
    let sha = resp.sha256.trim();
    if sha.len() != 64 || !sha.chars().all(|c| c.is_ascii_hexdigit()) {
        bail!("Invalid update manifest: sha256 must be 64 hex chars");
    }
    if !resp.version.chars().all(|c| c.is_ascii_digit() || c == '.') {
        bail!("Invalid update manifest: unsupported version format");
    }
...
pub fn check_software_update() {
    ...
    if config::option2bool(keys::OPTION_ENABLE_CHECK_UPDATE, &opt) {
        crate::updater::set_update_action_state("checking", "");
        std::thread::spawn(move || allow_err!(do_check_software_update()));
    }
}

// No need to check `danger_accept_invalid_cert` for now.
// Because the manifest url is fixed and served from the official UpDesk host.
#[tokio::main(flavor = "current_thread")]
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let url = get_software_update_manifest_url();
    let channel = get_software_update_channel();
    ...
    log::info!(
        "update check started: local_version={} channel={} manifest_url={}",
        crate::VERSION,
        channel,
        url
    );
    ...
    if get_version_number(&resp.version) > get_version_number(crate::VERSION) {
        #[cfg(feature = "flutter")]
        {
            let mut m = HashMap::new();
            m.insert("name", "check_software_update_finish");
            m.insert("url", &resp.url);
            m.insert("version", &resp.version);
            m.insert("changelog", &resp.changelog);
            m.insert(
                "mandatory",
                if resp.mandatory { "true" } else { "false" },
            );
            m.insert("min_supported", &resp.min_supported);
            if let Ok(data) = serde_json::to_string(&m) {
                let _ = crate::flutter::push_global_event(crate::flutter::APP_TYPE_MAIN, data);
            }
        }
        *SOFTWARE_UPDATE_URL.lock().unwrap() = resp.url.clone();
        *SOFTWARE_UPDATE_MANIFEST.lock().unwrap() = Some(resp);
        crate::updater::set_update_action_state("available", "");
    } else {
        #[cfg(feature = "flutter")]
        {
            let mut m = HashMap::new();
            ...
            m.insert("version", "");
            m.insert("changelog", "");
            m.insert("mandatory", "false");
            m.insert("min_supported", "");
            if let Ok(data) = serde_json::to_string(&m) {
                let _ = crate::flutter::push_global_event(crate::flutter::APP_TYPE_MAIN, data);
            }
        }
        *SOFTWARE_UPDATE_URL.lock().unwrap() = "".to_string();
        *SOFTWARE_UPDATE_MANIFEST.lock().unwrap() = None;
        crate::updater::set_update_action_state("up-to-date", "");
    }
    Ok(())
}

pub fn get_software_update_manifest() -> Option<SoftwareUpdateManifest> {
    SOFTWARE_UPDATE_MANIFEST.lock().unwrap().clone()
}

pub fn get_software_update_manifest_url() -> String {
    match get_software_update_channel().as_str() {
        "beta" => SOFTWARE_UPDATE_BETA_MANIFEST_URL.to_owned(),
        "recommended" => SOFTWARE_UPDATE_RECOMMENDED_MANIFEST_URL.to_owned(),
        _ => SOFTWARE_UPDATE_MANIFEST_URL.to_owned(),
    }
}
...
#[inline]
pub fn is_uptimedesk() -> bool {
    hbb_common::config::APP_NAME.read().unwrap().eq("UptimeDesk")
}
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    "https://admin.updesk.it".to_owned()
}

#[inline]
pub fn is_public(url: &str) -> bool {
    url.contains("updesk.it/") || url.ends_with("updesk.it")
}
...
pub fn check_process(arg: &str, mut same_uid: bool) -> bool {
    ...
    for (_, p) in sys.processes().iter() {
        ...
        // on mac, p.cmd() get "/Applications/UptimeDesk.app/Contents/MacOS/UptimeDesk", "XPC_SERVICE_NAME=com.carriez.UptimeDesk_server"
...
pub type UptimeDeskInterval = ThrottledInterval;

#[inline]
pub fn uptimedesk_interval(i: Interval) -> ThrottledInterval {
    ThrottledInterval::new(i)
}

pub fn load_custom_client() {
    // Hardcoded UptimeDesk server — not user-configurable
    {
        let mut s = hbb_common::config::OVERWRITE_SETTINGS.write().unwrap();
        s.insert("custom-rendezvous-server".to_owned(), "updesk.uptimeservice.it".to_owned());
        s.insert("relay-server".to_owned(), "updesk.uptimeservice.it".to_owned());
        s.insert("key".to_owned(), "tec7g45efBCxykMbBU8rozj0rVEq0Yp+Zz8MjjiN4A4=".to_owned());
        s.insert("allow-websocket".to_owned(), "Y".to_owned());
    }
    // Hide server/network settings from client UI
    {
        let mut b = hbb_common::config::BUILTIN_SETTINGS.write().unwrap();
        b.insert("hide-network-settings".to_owned(), "Y".to_owned());
        b.insert("hide-server-settings".to_owned(), "Y".to_owned());
        b.insert("hide-proxy-settings".to_owned(), "Y".to_owned());
        b.insert("hide-websocket-settings".to_owned(), "Y".to_owned());
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_UptimeDesk_interval() {
        ...
        for maker in base_intervals.into_iter() {
            ...
            let mut timer = uptimedesk_interval(maker());
    ...
    #[test]
    fn test_get_software_update_manifest_url_by_channel() {
        LocalConfig::set_option("update-channel".to_owned(), "stable".to_owned());
        assert_eq!(
            get_software_update_manifest_url(),
            SOFTWARE_UPDATE_MANIFEST_URL
        );
        LocalConfig::set_option("update-channel".to_owned(), "recommended".to_owned());
        assert_eq!(
            get_software_update_manifest_url(),
            SOFTWARE_UPDATE_RECOMMENDED_MANIFEST_URL
        );
        LocalConfig::set_option("update-channel".to_owned(), "beta".to_owned());
        assert_eq!(
            get_software_update_manifest_url(),
            SOFTWARE_UPDATE_BETA_MANIFEST_URL
        );
        LocalConfig::set_option("update-channel".to_owned(), "".to_owned());
    }
    ...
    #[allow(non_snake_case)]
    #[tokio::test]
    async fn test_UptimeDesk_interval_sleep() {
        let base_intervals = [interval_maker, interval_at_maker];

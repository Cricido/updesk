...
pub fn main_get_common(key: String) -> String {
    if key == "is-printer-installed" {
    ...
    } else if key == "software-update-url" {
        return crate::SOFTWARE_UPDATE_URL.lock().unwrap().clone();
    } else if key == "software-update-version" {
        return crate::common::get_software_update_manifest()
            .map(|m| m.version)
            .unwrap_or_default();
    } else if key == "software-update-changelog" {
        return crate::common::get_software_update_manifest()
            .map(|m| m.changelog)
            .unwrap_or_default();
    } else if key == "software-update-mandatory" {
        return crate::common::get_software_update_manifest()
            .map(|m| if m.mandatory { "true" } else { "false" }.to_owned())
            .unwrap_or_default();
    } else if key == "software-update-min-supported" {
        return crate::common::get_software_update_manifest()
            .map(|m| m.min_supported)
            .unwrap_or_default();
    } else if key == "software-update-status" {
        return crate::updater::get_update_action_status();
    } else if key == "software-update-error" {
        return crate::updater::get_update_action_error();
        ...
        if key.starts_with("download-data-") {
            ...
            return match (
                ...
                (Ok(true), false) => format!("uptimedesk-{_version}-x86_64.msi"),
                (Ok(true), true) | (Ok(false), _) => format!("uptimedesk-{_version}-x86_64.exe"),
                (Err(e), _) => {
            ...
            #[cfg(target_os = "macos")]
            {
                return if cfg!(target_arch = "x86_64") {
                    format!("uptimedesk-{_version}-x86_64.dmg")
                } else if cfg!(target_arch = "aarch64") {
                    format!("uptimedesk-{_version}-aarch64.dmg")
                } else {
...
pub fn main_set_common(_key: String, _value: String) {
    ...
    #[cfg(any(target_os = "windows", target_os = "macos"))]
    {
        ...
        if _key == "download-new-version" {
            ...
            #[cfg(target_os = "windows")]
            match crate::updater::launch_update_from_url(&_value) {
                Ok(_) => {

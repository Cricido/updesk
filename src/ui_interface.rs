...
use crate::common::get_software_update_manifest;
#[cfg(feature = "flutter")]
...
async fn check_connect_status_(reconnect: bool, rx: mpsc::UnboundedReceiver<ipc::Data>) {
    ...
    loop {
        if let Ok(mut c) = ipc::connect(1000, "").await {
            let mut timer = crate::uptimedesk_interval(time::interval(time::Duration::from_secs(1)));
            loop {
                tokio::select! {
                    res = c.next() => {
                        match res {

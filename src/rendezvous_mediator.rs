...
impl RendezvousMediator {
    ...
    pub async fn start_udp(server: ServerPtr, host: String) -> ResultType<()> {
        ...
        let mut timer = crate::uptimedesk_interval(interval(crate::TIMER_OUT));
        ...
        const MAX_FAILS_SWITCH: i64 = 6; // bail to TCP/WebSocket after 6 UDP failures (~18s)
            ...
            select! {
                ...
                _ = timer.tick() => {
                    ...
                    if timeout || (last_register_sent.is_none() && expired) {
                        if timeout {
                            fails += 1;
                            if fails >= MAX_FAILS_SWITCH {
                                bail!("UDP_FAILED_SWITCH_TO_TCP");
                            } else if fails >= MAX_FAILS2 {
            ...
            Some(rendezvous_message::Union::RegisterPkResponse(rpr)) => {
                ...
                match rpr.result.enum_value() {
                    ...
                    Ok(register_pk_response::Result::NOT_SUPPORT) => {
                        log::info!(
                            "RegisterPkResponse NOT_SUPPORT from {}, fallback to legacy register_peer",
                            self.host
                        );
                        Config::set_host_key_confirmed(&self.host_prefix, true);
                        *SOLVING_PK_MISMATCH.lock().await = "".to_owned();
                    ...
                    _ => {
                        // hbbs <=1.1.15 uses UUID_MISMATCH=1 (newer proto has UUID_MISMATCH=2)
                        if rpr.result.value() == 1 {
                            log::info!("RegisterPkResponse result=1 (legacy UUID_MISMATCH), regenerating ID");
                            self.handle_uuid_mismatch(sink).await?;
                        } else {
                            log::error!("unknown RegisterPkResponse: result={}", rpr.result.value());
                        }
                    }
                }
                if rpr.keep_alive > 0 {
    ...
    pub async fn start(server: ServerPtr, host: String) -> ResultType<()> {
            ...
            || crate::is_udp_disabled()
        {
            ...
            match Self::start_udp(server.clone(), host.clone()).await {
                Err(ref e) if e.to_string().contains("UDP_FAILED_SWITCH_TO_TCP") => {
                    log::info!("UDP unreachable, switching to TCP/WebSocket for {}", host);
                    Self::start_tcp(server, host).await
                }
                other => other,
            }

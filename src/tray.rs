...
fn make_tray() -> hbb_common::ResultType<()> {
    ...
    // Status icons: green = online/idle, orange = session active
    #[cfg(windows)]
    let icon_online =
        tray_icon::Icon::from_rgba(make_dot_icon(0x3C, 0xC8, 0x70), 32, 32).ok();
    #[cfg(windows)]
    let icon_connected =
        tray_icon::Icon::from_rgba(make_dot_icon(0xFF, 0x8C, 0x00), 32, 32).ok();
    ...
    #[cfg(windows)]
    let mut last_session_count: usize = 0;
    #[cfg(windows)]
        ...
        if let Ok(data) = ipc_receiver.try_recv() {
            match data {
                Data::ControlledSessionCount(count) => {
                    ...
                    // Toast: notify only when a new connection arrives
                    if baseline_established && count > last_session_count {
                        tauri_winrt_notification::Toast::new(
                            tauri_winrt_notification::Toast::POWERSHELL_APP_ID,
                        )
                        .title("UptimeDesk")
                        .text1("Nuovo collegamento remoto")
                    ...
                    // Icon badge: green = idle, orange = active session
                    let new_icon = if count > 0 {
                        icon_connected.clone()
                    } else {
                        icon_online.clone()
                    };
                    if let Some(ic) = new_icon {
                        _tray_icon.lock().unwrap().as_mut().map(|t| t.set_icon(Some(ic)));
                    }
...
async fn start_query_session_count(sender: std::sync::mpsc::Sender<Data>) {
    ...
    loop {
        if let Ok(mut c) = crate::ipc::connect(1000, "").await {
            let mut timer = crate::uptimedesk_interval(tokio::time::interval(Duration::from_secs(1)));
            loop {
                tokio::select! {
                    res = c.next() => {
                        match res {
...
#[cfg(windows)]
fn make_dot_icon(r: u8, g: u8, b: u8) -> Vec<u8> {
    let size = 32u32;

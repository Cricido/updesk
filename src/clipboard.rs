...
const UPTIMEDESK_CLIPBOARD_OWNER_FORMAT: &'static str = "dyn.com.rustdesk.owner";
...
const SUPPORTED_FORMATS: &[ClipboardFormat] = &[
    ...
    ClipboardFormat::Special(UPTIMEDESK_CLIPBOARD_OWNER_FORMAT),
];
...
pub fn is_file_url_set_by_uptimedesk(url: &Vec<String>) -> bool {
    if url.len() != 1 {
        return false;
    }
    url.iter()
        .next()
        .map(|s| {
            for prefix in &["file:///tmp/.uptimedesk_", "//tmp/.uptimedesk_"] {
...
fn do_update_clipboard_(mut to_update_data: Vec<ClipboardData>, side: ClipboardSide) {
    ...
    if let Some(ctx) = ctx.as_mut() {
        to_update_data.push(ClipboardData::Special((
            UPTIMEDESK_CLIPBOARD_OWNER_FORMAT.to_owned(),
            side.get_owner_data(),
        )));
...
impl ClipboardContext {
    ...
    fn get_formats_filter(
        ...
        if !force {
            for c in data.iter() {
                if let ClipboardData::Special((s, d)) = c {
                    if s == UPTIMEDESK_CLIPBOARD_OWNER_FORMAT && side.is_owner(d) {
                        return Ok(vec![]);
                    }
                }
            }
        ...
        Ok(data
            .into_iter()
            .filter(|c| match c {
                ClipboardData::Special((s, _)) => s != UPTIMEDESK_CLIPBOARD_OWNER_FORMAT,
    ...
    #[cfg(all(feature = "unix-file-copy-paste", target_os = "macos"))]
    fn get_file_urls_set_by_uptimedesk(
        data: Vec<ClipboardData>,
        _side: ClipboardSide,
    ) -> Vec<String> {
        for item in data.into_iter() {
            if let ClipboardData::FileUrl(urls) = item {
                if is_file_url_set_by_uptimedesk(&urls) {
                    return urls;
                }
            }
        }
    ...
    fn get_file_urls_set_by_uptimedesk(data: Vec<ClipboardData>, side: ClipboardSide) -> Vec<String> {
        let exclude_path =
    ...
    fn try_empty_clipboard_files(&mut self, side: ClipboardSide) {
        ...
        if let Ok(data) = self.get_formats(&[ClipboardFormat::FileUrl]) {
            let urls = Self::get_file_urls_set_by_uptimedesk(data, side);
            if !urls.is_empty() {
                // FIXME:
                ...
                let clear_holder_text = if is_kde_x11 {
                    "UptimeDesk placeholder to clear the file clipboard"
                } else {
                    ""
                }
                .to_string();

...
impl VirtualInputState {
    fn new() -> Option<Self> {
        VirtualInput::new(
            ...
            // 1. Press "Command" key in UptimeDesk
            // 2. Move the physical mouse
            // 3. Press "V" key in UptimeDesk
            ...
            // we consider only UptimeDesk operates the controlled side.
            //
...
fn key_sleep() {
    ...
    // Though `/Applications/UptimeDesk.app/Contents/MacOS/rustdesk --server` in terminal is ok.
    let now = Instant::now();
...
pub async fn lock_screen() {
    ...
    if #[cfg(target_os = "linux")] {
        ...
        // loginctl lock-session also not work, they both work run uptimedesk from cmd
        std::thread::spawn(|| {

 ...
 */
/// Converts a TCP endpoint to its WebSocket equivalent (always, ignoring use_ws() setting).
/// Returns None if the endpoint is empty, already a WS endpoint, or cannot be parsed.
pub fn to_ws_unconditional(endpoint: &str) -> Option<String> {
    if endpoint.is_empty() || is_ws_endpoint(endpoint) {
        return None;
    }
    let (endpoint_host, endpoint_port) = split_host_port(endpoint)?;
...
pub fn check_ws(endpoint: &str) -> String {
    ...
    to_ws_unconditional(endpoint).unwrap_or_else(|| endpoint.to_string())
}

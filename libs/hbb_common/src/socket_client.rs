...
use crate::{
    ...
    tcp::FramedStream,
    udp::FramedSocket,
    websocket::{self, check_ws, is_ws_endpoint, to_ws_unconditional},
    ResultType, Stream,
};
...
// Connects via TCP and WebSocket in parallel, returning whichever succeeds first.
// This mirrors the "happy eyeballs" approach used by TeamViewer/AnyDesk:
// - Fast/open networks: TCP wins immediately, WebSocket future is dropped.
// - Firewall-restricted networks: TCP fails, WebSocket on port 443 wins.
// - Zero added latency in either case.
...
>(
    ...
    // If already a WebSocket endpoint, connect directly (no parallel needed).
    let ws_check = check_ws(&target_str);
    if is_ws_endpoint(&ws_check) {
        return Ok(Stream::WebSocket(
            websocket::WsFramedStream::new(ws_check, None, None, ms_timeout).await?,
        ));
    }

    // Compute WebSocket URL. If not computable, fall back to plain TCP.
    let ws_url = match to_ws_unconditional(&target_str) {
        Some(url) => url,
        None => return connect_tcp_local(target, None, ms_timeout).await,
    };

    // Race TCP and WebSocket in parallel — first success wins.
    let tcp_str = target_str.clone();
    let ws_url_clone = ws_url.clone();

    let tcp_fut = async move { connect_tcp_local(tcp_str, None, ms_timeout).await };
    let ws_fut = async move {
        websocket::WsFramedStream::new(ws_url_clone, None, None, ms_timeout)
            .await
            .map(Stream::WebSocket)
    };

    // Pin both futures and race them; keep the slower one running until a winner is found.
    tokio::pin!(tcp_fut);
    tokio::pin!(ws_fut);
    ...
    let mut ws_err: Option<anyhow::Error> = None;

    loop {
        tokio::select! {
            result = &mut tcp_fut, if !tcp_done => {
                tcp_done = true;
                match result {
                    Ok(stream) => {
                        log::debug!("connect_tcp: TCP won for {}", target_str);
                        return Ok(stream);
                    }
                    Err(e) => {
                        log::info!("connect_tcp: TCP failed for {} ({}), waiting for WebSocket", target_str, e);
                        tcp_err = Some(e);
                    }
                }
            }
            result = &mut ws_fut, if !ws_done => {
                ws_done = true;
                match result {
                    Ok(stream) => {
                        log::debug!("connect_tcp: WebSocket won for {}", target_str);
                        return Ok(stream);
                    }
                    Err(e) => {
                        log::info!("connect_tcp: WebSocket failed for {} ({})", target_str, e);
                        ws_err = Some(e);
                    }
                }
            }
    ...
    Err(tcp_err
        .or(ws_err)
        .unwrap_or_else(|| anyhow::anyhow!("Both TCP and WebSocket connections failed")))
}
